...
import { gitConflictsChallenges } from "../data/challenges";
...
export default function GitConflictsChallenge() {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [score, setScore] = useState(0);
  const [answers, setAnswers] = useState<boolean[]>([]);
  const [isFinished, setIsFinished] = useState(false);

  const currentChallenge = gitConflictsChallenges[currentIndex];
  const totalCases = gitConflictsChallenges.length;
        ...
        {!isFinished ? (
          ...
          >
            <DetectiveResults
              ...
              total={totalCases}
              userAnswers={answers}
              challenges={gitConflictsChallenges}
