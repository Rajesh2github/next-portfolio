...
import { seoChallenges } from "../data/challenges";
...
export default function SeoChallenge() {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [score, setScore] = useState(0);
  const [answers, setAnswers] = useState<boolean[]>([]);
  const [isFinished, setIsFinished] = useState(false);

  const currentChallenge = seoChallenges[currentIndex];
  const totalCases = seoChallenges.length;
